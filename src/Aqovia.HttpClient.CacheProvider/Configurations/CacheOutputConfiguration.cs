...
using Aqovia.HttpClient.CacheProvider.KeyGenerators;
...
namespace Aqovia.HttpClient.CacheProvider.Configurations
{
    public class CacheOutputConfiguration
    {
        ...
        public void RegisterCacheOutputProvider(Func<ICacheOutput> provider)
        {
            _configuration.Properties.GetOrAdd(typeof(ICacheOutput), x => provider);
        }
        ...
        public ICacheOutput GetCacheOutputProvider(HttpRequestMessage request)
        {
            object cache;
            _configuration.Properties.TryGetValue(typeof(ICacheOutput), out cache);

            var cacheFunc = cache as Func<ICacheOutput>;

            var cacheOutputProvider = cacheFunc != null ? cacheFunc() : request.GetDependencyScope().GetService(typeof(ICacheOutput)) as ICacheOutput ?? new InMemoryCache();
